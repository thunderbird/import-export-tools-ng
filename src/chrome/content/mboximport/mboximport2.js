...
async function testBuildMbox(msgFolder, dest) {
	...
	const getMsgLoop = async (emlsArray, startIndex) => {
		...
		while (folderMsgs.hasMoreElements()) {
			...
			try {
				fromAddr2 = parse5322.parseFrom(msgHdr.author)[0].address;
				} catch (ex) {
					fromAddr2 = "";
				}
			...
			/*
			console.log(msgUri)
			console.log(msgHdr.author)
			console.log(msgHdr.dateInSeconds * 1000)
			console.log(new Date(msgHdr.dateInSeconds * 1000))
